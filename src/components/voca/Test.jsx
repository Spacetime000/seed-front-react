...
const Test = () => {
  ...
  const fetchData = useCallback(() => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/review/${params.id}`, {
        headers: {
            Authorization: "Bearer " + localStorage.getItem("accessToken"),
          },
    }).then((res) => {
        setContents(res.data.contents);
        setMaxIndex(res.data.contents.length);
  ...
  const handleReviewClick = (day) => {
    ...
    axios.patch(`${process.env.REACT_APP_BASE_URL}/review`, data, {
      headers: {
        Authorization: "Bearer " + localStorage.getItem("accessToken"),
      },
    ...
    >
        ...
        <Box>
          ...
          <Box sx={{
            ...
            <Button color="success" onClick={() => handleReviewClick(5)}>쉬움 : {count * 5}일 뒤</Button>
            <Button color="primary" onClick={() => handleReviewClick(3)}>보통 : {count * 3}일 뒤</Button>
            <Button color="danger" onClick={() => handleReviewClick(1)}>어려움 : {count * 1}일 뒤</Button>
            <Button color="neutral" component={Link} underline="none" href="/voca/my-page">목록</Button>
          </Box>
        </Box>
      ) : maxIndex === 0 ? (
